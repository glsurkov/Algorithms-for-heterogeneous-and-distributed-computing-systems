...
kernel void matrix_times_matrix(global float* a,
    ...
    int a_begin = n * local_size * group_id_1;
    int a_end   = a_begin + n - 1;
    int a_step  = local_size;
    int b_begin = local_size * group_id_0;
    int b_step  = local_size * n;
    
    float resultSub = 0.0f;

    for (int i = a_begin, j = b_begin; i <= a_end; i += a_step, j += b_step) {
        loc_block_a[local_id_0 + local_size*local_id_1] = a[i + n*local_id_1 + local_id_0];
        loc_block_b[local_id_0 + local_size*local_id_1] = b[j + n*local_id_1 + local_id_0];

        barrier(CLK_LOCAL_MEM_FENCE);

        for (int k = 0; k < local_size; ++k) {
            resultSub += loc_block_a[k + local_size*local_id_1] * loc_block_b[local_id_0 + local_size*k];
        }

        barrier(CLK_LOCAL_MEM_FENCE);
    }

    result[get_global_id(1) * n + get_global_id(0)] = resultSub;
}
)";
